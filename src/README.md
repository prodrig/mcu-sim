...
| F4 | DMA1/2, USART, TIM avanzados, EXTI/SYSCFG | **DMA y UART/USART completados**; TIM y EXTI/SYSCFG pendientes |
...
`make test` compila y ejecuta la suite de verificación acumulada (357
comprobaciones autocomprobables: 124 de F1 + 12 de F2 + 80 de F3 + 71 del DMA y
70 de UART/USART de F4; código de salida 0 si todas pasan, en menos de 2 s). Verificado con SystemC 2.3.4 / g++ 13 /
C++17 y arm-none-eabi-gcc 13.2.

Las pruebas T15-T17, T25, T31 y T37 necesitan los firmwares del repositorio; se
compilan con `make -C verif/fw`, `make -C verif/fw/coremark`,
`make -C verif/fw/blinky`, `make -C verif/fw/dma_demo` y
`make -C verif/fw/uart_demo` (requieren `arm-none-eabi-gcc`). Sin ellos, esas pruebas informan de que falta
