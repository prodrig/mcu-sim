...
`make test` compila y ejecuta la suite de verificación acumulada (**2047**
comprobaciones autocomprobables, y `make test446` las **101** del segundo chip —
...
que el modelo todavía no hace —los seis periféricos nuevos no están, los
escalones de tensión no limitan la frecuencia, `RCC_CKGATENR` se guarda y no
hace nada—, con la fase del plan en la que le toca a cada cosa. Un modelo
...
| :--- | :--- | :--- |
| Linux, g++ 13 | **verificado** | 2047/2047 comprobaciones, 101/101 del F446, `make red` 13/13, ASan limpio |
