...
{
    /* Consola virtual del modelo SystemC: buzon en la SRAM que el banco de
       pruebas vuelca al terminar la simulacion. No corresponde a hardware del
       STM32F407: en la fase F4 se sustituira por el USART real. */
    ...
    if (cm_console.len < sizeof(cm_console.buf) - 1)
        cm_console.buf[cm_console.len++] = c;
}
