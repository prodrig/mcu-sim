...
public:
    ...
    uint32_t conectar_swd() {
        ...
        uint32_t id = 0;
        leer_dp(0x0, id);
        // Encendido de los dominios de depuracion y del sistema.
    ...
    bool leer_dp(unsigned a, uint32_t& v)  { return paquete(false, false, a, v); }
    bool escribir_dp(unsigned a, uint32_t v) { return paquete(true, false, a, v); }
    bool leer_ap(unsigned a, uint32_t& v)  { return paquete(false, true, a, v); }
    bool escribir_ap(unsigned a, uint32_t v) { return paquete(true, true, a, v); }
    ...
    // Un bloque con auto-incremento: una sola escritura de TAR para todo.
    unsigned mem_read_block(uint32_t addr, uint32_t* dst, unsigned n) {
        escribir_ap(0x0, 0x23000052u);                // CSW: palabra, incremento
        escribir_ap(0x4, addr);
        unsigned k = 0;
        uint32_t basura = 0;
        if (!leer_ap(0xC, basura)) return 0;
        for (; k < n; ++k) {
            uint32_t v = 0;
            if (k + 1 < n) { if (!leer_ap(0xC, v)) break; }
...
private:
    ...
    bool paquete(bool escritura, bool ap, unsigned a, uint32_t& dato) {
        ...
        if (ack != 1u) { ++n_mal_; ciclo(-1); reposo(); return false; }   // 001 = OK
        ++n_ok_;
        if (!escritura) {
            uint32_t v = 0;
