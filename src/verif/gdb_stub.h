...
//      DHCSR, KEY en FP_CTRL) y su orden.
...
SC_MODULE(GdbStub) {
    ...
    GdbStub(sc_core::sc_module_name nm, analog_net_if& swclk, analog_net_if& swdio,
        ...
        : sc_core::sc_module(nm), swd_(swclk, swdio, swd_hz), puerto_(puerto) {
    ...
    void set_enabled(bool on) {
        if (on && !anunciado_) {
            anunciado_ = true;
            std::printf("[gdb] escuchando en localhost:%u\n", puerto_);
            std::fflush(stdout);
        }
        activo_ = on;
...
private:
    // =======================================================================
    // El lado de los pines: operaciones de alto nivel sobre el DAP
    // =======================================================================
    ...
    bool enganchar() {
        ...
        enganchado_ = (id == 0x2BA01477u) && swd_.encendido();
        if (enganchado_) {
            // C_DEBUGEN es la llave maestra: sin él, ni BKPT para el núcleo ni
            // el FPB sirve para nada.
    ...
    bool leer(uint32_t a, uint32_t& v)  { return swd_.mem_read32(a, v); }
    bool escribir(uint32_t a, uint32_t v) { return swd_.mem_write32(a, v); }
    ...
    // secuencia entera por SWD, exactamente como un ST-LINK.
    // =======================================================================
    ...
    void aceptar() {
        ...
        rx_.clear();
        sin_ack_ = false;
        std::printf("[gdb] cliente conectado\n"); std::fflush(stdout);
        if (!enganchado_) enganchar();
        if (halt_al_conectar_) { parar(); }
    }
    void desconectar() {
        if (cli_ >= 0) { ::close(cli_); cli_ = -1; }
        std::printf("[gdb] cliente desconectado\n"); std::fflush(stdout);
    }
    ...
    void cmd_leer_mem(const std::string& p) {
        ...
        if (nw && swd_.mem_read_block(base, w.data(), nw) != nw) {
            responder("E01");
            return;
        }
    ...
    void run() {
        if (!abrir()) {
            std::printf("[gdb] no se pudo abrir el puerto %u\n", puerto_);
            return;
        }
        for (;;) {
            wait(poll_, ev_);
    ...
    bool     enganchado_ = false, halt_al_conectar_ = true, activo_ = true;
    unsigned n_paq_ = 0, n_flash_ = 0, espera_ = 0;
