// =============================================================================
// crc_rng.h — Unidad de cálculo CRC (AHB1) y generador de números aleatorios
//             (AHB2) [IR, §12.19 y §12.20]
//
...
//     RUIDO que entrega un bit por ciclo de RNGCLK y un REGISTRO DE
//     DESPLAZAMIENTO REALIMENTADO que lo digiere; cada 40 ciclos se cosecha una
//     palabra. De ahí salen solas la cadencia, la bandera DRDY y —lo que
//     importa— las DOS condiciones de error del bloque, que son propiedades de
//     la fuente y del reloj, no banderas puestas a mano.
//
// Sobre la parametrización por rasgos que usa el resto del proyecto (USART/UART,
// los seis TIM, los cinco SPI/I2S, los tres I2C, los tres ADC, los dos canales
// de DAC, el SDIO): aquí NO se aplica, y conviene decir por qué. De cada uno de
// estos dos bloques el F407 lleva UNA sola instancia, y ninguno de los dos tiene
// ejes de configuración en este silicio: el polinomio del CRC está cableado
// (0x04C11DB7), el valor inicial también (0xFFFF FFFF), no hay inversión de bits
// ni de entrada ni de salida, y el RNG no tiene más ajuste que encenderlo.
// Montar una `struct` de rasgos para describir un espacio con un solo punto
// sería maquinaria sin contrapartida. Lo que sí se ha hecho es dejar el
// polinomio y el valor inicial como constantes con nombre en un solo sitio, que
// es donde entraría el eje si algún día hiciera falta modelar la unidad CRC
// programable de las familias posteriores.
//
// Implementado en la fase F5:
//   * CRC: CRC_DR (acumulación y lectura del resultado), CRC_IDR (8 bits, ajeno
//     al cálculo y al RESET del bloque) y CRC_CR.RESET, con su coste de un ciclo
//     de HCLK por palabra [IR, §12.20];
//   * RNG: RNG_CR (IE, RNGEN), RNG_SR (DRDY, CECS/CEIS, SECS/SEIS con su
//     semántica rc_w0), RNG_DR, cadencia de 40 ciclos de RNGCLK, error de reloj
//     por PLL48CK < HCLK/16, error de semilla por ruido atascado, e IRQ 80
//     [IR, §12.19].
// =============================================================================
