...
| **I-27** | **El stub de GDB no se hace pasar por un ST-LINK, y el camino por omisión de STM32CubeIDE es justamente ese.** Con `ST-LINK (ST-LINK GDB server)` apuntado al puerto del simulador, la capa de ST comprueba que al otro lado esté *su* servidor con una sonda de verdad, no lo encuentra y corta: «Could not verify ST device!». El stub responde bien —verificado con un cliente RSP crudo: `qSupported`, `?`, `g`, `m`, `target.xml`, y `DBGMCU_IDCODE` = `0x10016413`, el del F407— pero eso no le sirve a quien elige la sonda equivocada | analisis_gui §17 | **Hueco de producto, no de protocolo.** La vuelta es `GDB Hardware Debugging` con `Generic TCP/IP`, documentada en `src/README.md`, y funciona; pero **el objetivo del proyecto es que el alumno pruebe «igual que con la placa real»**, y el camino que un alumno toma por omisión en CubeIDE es el de ST. Las opciones son tres: (a) documentarlo y enseñarlo —lo hecho—; (b) suplantar el protocolo del servidor de ST, que no está publicado; (c) proveer un fichero de configuración de lanzamiento ya hecho para que el alumno no elija. **(c) es lo barato y lo que más se parece al objetivo** |
...
| **I** — Deuda de instrumentación y proyecto | 27 *(ocho cerradas: I-11, I-12, I-15, I-16, I-17, I-18, I-20 e I-22)* |
| **Total** | **141** |

De los 141, **uno solo** (P-01) es un pendiente de plan de primer orden; **once**
