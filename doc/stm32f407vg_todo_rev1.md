...
| **I-31** | **El SysTick sigue contando con el núcleo parado en depuración, y el tiempo simulado corre mientras tanto.** ARMv7-M dice que el contador del SysTick **no** decrementa con el procesador detenido en Debug state; en el modelo, `SysTick::tick_proc()` es un `SC_THREAD` que no sabe nada del `halt` | F6 | **Hueco conocido, y aquí pesa más que en el silicio.** En una placa real, pararse cinco segundos son cinco segundos de temporizadores. Aquí es peor: con el núcleo parado no queda casi nada que simular, así que el tiempo simulado vuela —el stub sondea el socket cada 100 µs simulados y los recorre a toda velocidad—, y mientras el alumno mira una variable pueden pasar *segundos* simulados. Al reanudar hay una interrupción esperando **siempre**. `C_MASKINTS` (I-30) arregla el paso a paso, que era el síntoma insoportable, pero no esto. Lo correcto es doble: que el SysTick no cuente con el núcleo parado, y que el avance del tiempo simulado se frene mientras nadie ejecuta (véase **I-25**) |
...
| **I** — Deuda de instrumentación y proyecto | 31 *(nueve cerradas: I-11, I-12, I-15, I-16, I-17, I-18, I-20, I-22 e I-28)* |
| **Total** | **145** |

De los 145, **uno solo** (P-01) es un pendiente de plan de primer orden; **once**
