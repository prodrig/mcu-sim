...
| **I-37** | **`--mcu TIPO` existe, pero solo hay un tipo que construir** | multi-MCU §6.1 | **Media tarea, y la mitad que falta es enorme.** `sim` acepta ya `--mcu` para elegir el tipo del MCU **implícito** —el que se monta cuando el XML no declara ninguno—, con `STM32F407VG` por omisión, insensible a mayúsculas y sin pisar lo que declare el XML. Lo que **no** existe es un segundo modelo: pedir `--mcu stm32f446re` da un error que nombra los tipos disponibles, y eso es lo correcto — simular un F407 mientras el usuario escribió F446RE sería exactamente la clase de mentira que este proyecto lleva toda la semana quitando. **Añadir un MCU no es un parámetro**: es otro mapa de memoria, otro juego de periféricos, otro encapsulado y otra tabla de funciones alternativas; lo que sí está hecho es que el sitio donde enchufarlo ya existe y se imprime solo (`TIPOS_MCU`, `--help`) |
...
| **I** — Deuda de instrumentación y proyecto | 39 *(diecinueve cerradas: I-11, I-12, I-15, I-16, I-17, I-18, I-20, I-22, I-25, I-28, I-30, I-31, I-32, I-33, I-34, I-35, I-36, I-38 e I-39)* |
| **Total** | **153** |

De los 153, **uno solo** (P-01) es un pendiente de plan de primer orden; **once**
