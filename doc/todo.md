...
| **T-22** | **El tiempo simulado total depende de los FIRMWARES, que no se versionan.** `2336217899213 ps` se mantuvo al picosegundo doce fases; en Windows sale `2334217899213 ps`, **2 ms menos**. La causa **no** es el anfitrión: los `.bin` se compilan en cada máquina con el compilador cruzado que allí haya —aquí `arm-none-eabi-gcc` 13.2 de Debian, allí el 13.3.1 de STM32CubeIDE—, un binario distinto ejecuta un número distinto de instrucciones, y **T17 sondea el final de CoreMark cada 2 ms**, así que esa diferencia sale **cuantizada a 2 ms**. **Medido en una sola máquina**: recompilando CoreMark de `-O2` a `-O1`, el total se mueve **24 ms exactos**, doce pasos del bucle. La suite publica ahora la **huella FNV-1a de `coremark.bin`**: dos totales solo son comparables si la huella coincide. **Confirmado entre las dos maquinas**: el `coremark.bin` de Linux mide **14528 B**, huella **`0x644FCE21`**; el de Windows mide **14856 B**, huella **`0x5157F2C7`**. Son binarios distintos, luego la comparacion de totales nunca fue legitima. *(La primera hipótesis —que la culpa era del sondeo del socket de GDB— quedó **refutada** por la propia medida que se añadió para comprobarla: T96+T97 consumen `96665875 ns` **idénticos** en las dos plataformas.)* | F6-gdb2 §8 |
