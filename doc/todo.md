...
| **T-23** | **El IWDG resetea UN TICK ANTES de su plazo.** Con un plazo programado de 201,0 ms, el modelo resetea a los 200,0. Lo destapó la migración a SystemC 3.0.2 (I-24): allí el reset caía a los 201,1 ms —el primer sondeo después del plazo, o sea lo correcto— y aquí a los 200,0, y la diferencia entre las dos versiones eran **1,00125 ms del tiempo simulado total**. La causa de la diferencia era el redondeo (`common/guarda_tick.h`) y ya está corregida: ahora las dos versiones coinciden. **Lo que NO está corregido es el tick de más**, porque `cnt_now()` da el reset cuando la cuenta LLEGA a cero y el silicio lo da cuando pasa DE cero, y arreglarlo mueve el invariante. Nadie lo había visto porque la comprobación que lo mira, `check_near(dti, timeout_s, 0.05)`, tiene un 5 % de tolerancia y el error es del 0,5 %: **una comprobación diez veces más floja que el fallo que debía cazar** | F5-RTC/WDG, 2026-09-23 |
