...
| **I-50** | **Los informes de `doc/` pueden ser obra derivada de los manuales de ST y de ARM, y el repositorio es público** | licencia, 2026-09-22 | **MEDIO RESUELTO el 2026-09-23: los tres informes salen del árbol; siguen en la historia.** Lo hecho: `informe.md`, `informe_revisado.md` e `informe_instrucciones.md` se han movido a `doc/refs/stm32f407xx/` —al lado de los manuales de los que salieron— y `.gitignore` excluye esa carpeta. **El criterio, que es lo que había que decidir y no se había decidido**: se va lo que se escribió para SUSTITUIR al manual, se queda lo que se escribió para RAZONAR sobre él. No es una corazonada: `informe_reglas.md` ordena por escrito *«prohibido remitir a otro documento… reproduce el dato en el propio informe»* y *«no resumas»*, y `informe_general.md` dice que el equipo destinatario *«NO tendrá acceso a los manuales originales»*. Eso es reproducción, no comentario. Por la misma regla **se quedan** los planes de fase, las dos comparativas entre piezas —que citan por sección en vez de reproducir— y los cuatro ficheros de prompts, que son el método. **Lo que NO arregla esto**: los informes siguen en los 116 commits del historial público, recuperables con `git log`. Sacarlos de ahí exige reescribir la historia entera (`git filter-repo`, y ojo: hasta `97aabd1` vivían en `doc/*.md`, no en `doc/stm32f4xx/`), lo que cambia TODOS los hashes —incluidos los que cita esta misma documentación— y no borra lo que ya clonó o replicó alguien. Con un repositorio de cuatro días y sin forks, sale más limpio borrarlo en GitHub y volver a subirlo filtrado. **Esa decisión sigue pendiente.** **Y lo que sigue sin hacer**: leer las dos comparativas con esta pregunta en la mano. Se han quedado por criterio, no por lectura |
