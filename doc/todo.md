...
| **I-23** | ~~**Sin verificación en Windows ni en macOS**~~ **WINDOWS HECHO; macOS EN CAMINO.** | analisis_gui §19.3 | **Windows, cerrado**: las tres suites pasan allí, y desde que los firmwares se versionan dan **2118/204/165 con los tres tiempos simulados idénticos a los de Linux al picosegundo** (T-22). **macOS, ya no es un hueco sin plan**: entra por la integración continua (**P-13**), en las dos arquitecturas, con hardware Apple real —que es la única forma legal de probarlo desde un PC con Windows: la licencia de macOS solo permite virtualizarla sobre un ordenador Apple—. **macOS Apple Silicon, CERRADO el 2026-09-22**: la tercera ejecución del CI pasa las tres suites con los tres invariantes —el trabajo falla si no coinciden—, y además es la primera vez que salen con un **paquete de corrutinas distinto** (pthreads), lo que confirma que el tiempo lo lleva el planificador y nada más. **macOS Intel sigue abierto**: QuickThreads no enlaza en Mach-O y se ha pasado también a pthreads, sin ejecutar todavía. Queda por tanto **una** plataforma de cuatro |
