...
| **I-24** | **Se sigue en SystemC 2.3.4 (IEEE 1666-2011) pudiendo estar en la línea 3.0 (IEEE 1666-2023)** | analisis_systemc3 | **Decisión consciente, no olvido.** El inventario de la API que el modelo usa —`sc_module`, procesos, señales, `sc_prim_channel`, `sc_spawn` y TLM-2.0, nada más— es tan central que no se espera ningún cambio, y el único `#include` de cabeceras internas de SystemC ya se ha eliminado. Pero **ninguna ventaja conocida de la 3.0 ataca un problema que tengamos** —el coste lo domina el número de despertares, no el planificador— y sí tiene un coste: hoy la 2.3.4 se instala con `apt` y la 3.0 habría que compilarla. **La pregunta está subordinada a I-23**: quien construya SystemC para Windows y macOS que pruebe las dos versiones a la vez. **2026-09-22: ya hay un argumento concreto, y lo ha traído la integración continua el primer día.** El `CMakeLists.txt` de la 2.3.4 declara `cmake_minimum_required` por debajo de 3.5, y **CMake 4 ha retirado esa compatibilidad**: en macOS y en MSYS2 la configuración aborta. Se sigue con la 2.3.4 añadiendo `-DCMAKE_POLICY_VERSION_MINIMUM=3.5`, que es la salida que el propio CMake propone, pero eso es un parche con fecha de caducidad. La frase «ninguna ventaja conocida de la 3.0 ataca un problema que tengamos» ya no es cierta: el problema es que **2.3.4 se está quedando fuera de las cadenas de herramientas actuales**. **Comprobado sobre la etiqueta 3.0.2**, que de los tres fallos que la CI ha destapado arregla **uno**: su `cmake_minimum_required` es `3.5...3.31`, con lo que el parche de la política de CMake sobraría. Los otros dos **siguen igual**: `src/sysc/packages/qt/md/iX86_64.s` es byte a byte el mismo fichero, con la nota `.note.GNU-stack` bajo un `#if` que una extensión en minúscula impide evaluar; y `sc_cor_qt.cpp` conserva el mismo modismo `weakref` que no enlaza en Mach-O. **Lo que sí cambia el planteamiento** es que la 3.0 añade una tercera implementación de corrutinas, **`ENABLE_STD_THREADS`** (`sc_cor_std_thread.cpp`, sobre `std::thread`), y a diferencia de `ENABLE_PTHREADS` **no está prohibida en Windows**: la guarda `if (WIN32 AND ENABLE_PTHREADS) → FATAL_ERROR` sigue ahí y no tiene equivalente. Si funciona, sería **una sola opción, la misma en las cuatro plataformas**, en vez de las tres recetas distintas de hoy, dos de ellas rotas. Es una hipótesis: nadie la ha ejecutado. **LA PREGUNTA QUE IMPORTA NO ES NINGUNA DE ÉSTAS, SINO SI SE MUEVE EL INVARIANTE.** Pasar de 1666-2011 a 1666-2023 es cambiar el planificador, no la fontanería. La evidencia de hoy apunta a que no debería —el mismo tiempo simulado salió con **dos paquetes de corrutinas distintos**, QuickThreads en Linux y pthreads en macOS arm64—, pero eso es evidencia sobre el cambio de contexto, no sobre el planificador. Y hay un coste que no estaba contado: hoy el trabajo de Linux usa `libsystemc-dev` de Ubuntu, que es **la vía que documenta `compilacion.md` §4**; con la 3.0 habría que compilarla también allí y la CI dejaría de verificar las instrucciones que se le dan a un alumno. **El orden correcto es el contrario del que decía este punto.** No «probar las dos versiones a la vez»: **primero cerrar las cuatro plataformas con la 2.3.4**, y sólo entonces probar la 3.0.2 en una rama aparte, con un invariante estable de referencia. Así, si el invariante se mueve, se sabrá que fue SystemC y no otra de las cinco cosas que se tocan a la vez. Es el método que faltó en T-22 |
...
De los 173, **uno solo** (P-01) es un pendiente de plan de primer orden; **once**
